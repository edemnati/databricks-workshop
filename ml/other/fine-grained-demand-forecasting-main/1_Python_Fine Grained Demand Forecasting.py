...
train = spark.read.table("demande_forecast_train_shared"
  )
...
def evaluate_forecast( evaluation_pd: pd.DataFrame ) -> pd.DataFrame:
  ...
  # calculate evaluation metrics
  mae = mean_absolute_error( evaluation_pd['sales'], evaluation_pd['sales_predicted'] )
  mse = mean_squared_error( evaluation_pd['sales'], evaluation_pd['sales_predicted'] )
  rmse = sqrt( mse )
...
results = (
  spark
    .table('forecasts')
    .filter('date < \'2018-01-01\'') # limit evaluation to periods where we have historical data
    .select('training_date', 'store', 'item', 'sales', 'sales_predicted')
